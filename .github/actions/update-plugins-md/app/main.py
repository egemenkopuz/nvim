...
PLUGIN_LOCK_FILE_PATH: Path = Path("./plugin-lock.json")
PLUGINS_MD_FILE_PATH: Path = Path("./PLUGINS.md")
# fmt: on
...
class PluginDetails:
    def __init__(
        self, name: str = "", category: str = "", description: str = "", link: str = ""
    ) -> None:
        self.full_name = name
        ...
        with urlopen(url) as response:
            ...
            for line in response:
                ...
                if category_list_started and line.startswith("- ["):
                    ...
                    out[plugin_name] = PluginDetails(
                        plugin_name, curr_category, plugin_description, plugin_link
                    )
...
def get_installed_plugin_details(
    lock_file_path: Union[str, Path], verbose: bool = False
...
def run() -> None:
    ...
    installed_plugins: Dict[str, Any] = get_installed_plugin_details(
        args.lock_file_path, args.verbose
    )
    assert len(installed_plugins) > 0, "No plugins are installed on the system"

    plugin_details: Dict[str, PluginDetails] = get_online_plugin_details(
        args.details_url, args.verbose
    )
    assert (
        len(plugin_details) > 0
    ), f"No plugins details are retrieved from {args.details_url}"
    ...
    write_plugins_md(
        installed_plugins,
        args.plugins_md_file_path,
        args.verbose,
        args.disable_readme_ending,
    )
